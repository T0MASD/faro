...
// ⚠️  ARCHITECTURAL NOTE: This example predates the clean architecture refactor
// 
// ISSUES WITH THIS EXAMPLE:
// - Mixes Faro mechanisms with complex business logic
// - Implements CRD discovery, workload detection, and annotation processing
// - Contains policies that should be user responsibility, not library core
//
// RECOMMENDED: Use workload-monitor-clean.go instead, which properly demonstrates:
// - Faro Core: Provides mechanisms (informer management, event streaming, JSON export)
// - User Code: Implements policies (workload detection, business logic, workflows)
//
// This file is kept for compatibility but does not reflect current architecture best practices.
...
func (w *WorkloadMonitor) addWorkloadToUnifiedController(workloadID, workloadName string, namespaces []string) {
	...
	// Note: Informers are automatically started when the unified controller starts
		// Use the unified StartInformers() method - the one common denominator for everyone
...
func (w *WorkloadMonitor) extractGVRFromInvolvedObject(involvedObj map[string]interface{}) string {
	...
	// CORRECTED: Kubernetes API provides correct information - no special cases needed
	// Simple pluralization works because the API server validates GVRs
...
func (w *WorkloadMonitor) addGVRToWorkloadController(workloadID, newGVR string, namespaces []string) {
	...
	// Start the informer for the newly added resource - EVERYONE uses this same method
	if err := w.unifiedController.StartInformers(); err != nil {
		w.logger.Error("dynamic-discovery", "["+w.clusterName+"] Failed to start new informers for GVR '"+newGVR+"': "+err.Error())
		return
	}
