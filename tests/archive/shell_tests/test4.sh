...
main() {
    ...
    kubectl patch service test-service -n faro-test-1 --patch='{"metadata":{"annotations":{"phase3":"updated","test":"test4"}}}' 2>/dev/null || log "Service not found (expected in ConfigMap-only test)"
    kubectl patch secret test-secret -n faro-test-1 --patch='{"metadata":{"annotations":{"phase3":"updated","test":"test4"}}}' 2>/dev/null || log "Secret not found (expected in ConfigMap-only test)"
    ...
    if grep -q "CONFIG \[UPDATED\].*faro-test-1/test-service" "$log_file"; then
        ...
        log "Service UPDATED event not found (expected - test4 focuses on ConfigMaps)"
    fi

    if grep -q "CONFIG \[UPDATED\].*faro-test-1/test-secret" "$log_file"; then
        success "Secret UPDATED event detected!"
    else
        log "Secret UPDATED event not found (expected - test4 focuses on ConfigMaps)"
    fi
