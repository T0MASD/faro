...
# Simple E2E Test 2 - Resource-Centric ConfigMap
...
main() {
    log "Starting Simple E2E Test 2 - Resource-Centric Config"
    ...
    local log_file="logs/simple-test-2.log"

    log "Faro log file: $SCRIPT_DIR/$log_file"

    # Start Faro
    log "Starting Faro with resource-centric config..."
    timeout 60s $FARO_BIN -config="configs/simple-test-2.yaml" > "$log_file" 2>&1 &
    local faro_pid=$!
    ...
    kubectl patch service test-service -n faro-test-1 --patch='{"metadata":{"annotations":{"phase3":"updated"}}}'
    kubectl patch secret test-secret -n faro-test-1 --patch='{"metadata":{"annotations":{"phase3":"updated"}}}'
    ...
    if grep -q "CONFIG \[UPDATED\].*faro-test-1/test-service" "$log_file"; then
        ...
        log "Service UPDATED event not found (may not be monitored in this test)"
    fi

    if grep -q "CONFIG \[UPDATED\].*faro-test-1/test-secret" "$log_file"; then
        success "Secret UPDATED event detected!"
    else
        log "Secret UPDATED event not found (may not be monitored in this test)"
    fi

    # Delete Service and Secret before ConfigMaps
    ...
    sleep 2
    log "Checking for Service/Secret DELETED events..."

    if grep -q "CONFIG \[DELETED\].*faro-test-1/test-service" "$log_file"; then
        success "Service DELETED event detected!"
    else
        log "Service DELETED event not found (may not be monitored in this test)"
    fi

    if grep -q "CONFIG \[DELETED\].*faro-test-1/test-secret" "$log_file"; then
        success "Secret DELETED event detected!"
    else
        log "Secret DELETED event not found (may not be monitored in this test)"
    fi
